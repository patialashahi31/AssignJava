...
public class Assignment {
    ...
    void addBooking() {
       ...
       if (isBook(selectedRoom.getRoomNumber(),booking.getStart())) {
            System.out.println("*** Booking Successful ***");
            
            booking.setEnd(end);
            booking.setStartDay(checkinDay);
            booking.setEndDay(checkoutDay);
            booking.setStartMonth(checkoutMonth);
    ...
    private boolean isBook(int room, int start){
        int flag=0;
        for(Booking bv : bookings){
            if(bv.getRoomNumber()==room){
                flag=1;
                if(bv.getEnd()<start){
                    return true;
                }
